...
class HomeController extends Controller
{
    ...
    public function member_listing(Request $request)
    {
        ...
        $mother_tongue_id  = ($request->mother_tongue != null) ? $request->mother_tongue : null ;
      ...
      if($matital_status != null){
            if (count($matital_status) ==1){
                $user_ids = Member::where('marital_status_id', $matital_status[0])->pluck('user_id')->toArray();
               
                $users = $users->WhereIn('id', $user_ids);
            }
            if (count($matital_status) >1){
                $user_idss = Member::where('marital_status_id', $matital_status[0]);
                for ($x = 0; $x <(count($matital_status)); $x++) {
                   $user_idss = $user_idss->orWhere('marital_status_id', $matital_status[$x]);
                   }   
                 $user_ids = $user_idss->pluck('user_id')->toArray();
                $users = $users->WhereIn('id', $user_ids);
            }
        }
        ...
        elseif (!empty($caste_id)) {
            if (count($caste_id) ==1){
                $user_ids = SpiritualBackground::where('caste_id', $caste_id[0])->pluck('user_id')->toArray();
                $users = $users->WhereIn('id', $user_ids);
           }
           if (count($caste_id) >1){
               $user_idss = SpiritualBackground::where('caste_id', $caste_id[0]);
               for ($x = 0; $x <(count($caste_id)); $x++) {
                  $user_idss = $user_idss->orWhere('caste_id', $caste_id[$x]);
                  }   
                $user_ids = $user_idss->pluck('user_id')->toArray();
               $users = $users->WhereIn('id', $user_ids);
           }
        ...
        elseif(!empty($religion_id)) {
            if (count($religion_id) ==1){
                $user_ids = SpiritualBackground::where('religion_id', $religion_id[0])->pluck('user_id')->toArray();
                $users = $users->WhereIn('id', $user_ids);
           }
           if (count($religion_id) >1){
               $user_idss = SpiritualBackground::where('religion_id', $religion_id[0]);
               for ($x = 0; $x <(count($religion_id)); $x++) {
                  $user_idss = $user_idss->orWhere('religion_id', $religion_id[$x]);
                  }   
                $user_ids = $user_idss->pluck('user_id')->toArray();
               $users = $users->WhereIn('id', $user_ids);
           }
        ...
        if(!empty($city_id)){
           if (count($city_id) ==1){
                $user_ids = Address::where('city_id', $city_id[0])->pluck('user_id')->toArray();
                $users = $users->WhereIn('id', $user_ids);
           }
           if (count($city_id) >1){
               $user_idss = Address::where('city_id', $city_id[0]);
               for ($x = 0; $x <(count($city_id)); $x++) {
                  $user_idss = $user_idss->orWhere('city_id', $city_id[$x]);
                  }   
                $user_ids = $user_idss->pluck('user_id')->toArray();
               $users = $users->WhereIn('id', $user_ids);
           }
        ...
        elseif (!empty($state_id)) {
            if (count($state_id) ==1){
                $user_ids = Address::where('state_id', $state_id[0])->pluck('user_id')->toArray();
                $users = $users->WhereIn('id', $user_ids);
           }
           if (count($state_id) >1){
               $user_idss = Address::where('state_id', $state_id[0]);
               for ($x = 0; $x <(count($state_id)); $x++) {
                  $user_idss = $user_idss->orWhere('state_id', $state_id[$x]);
                  }   
                $user_ids = $user_idss->pluck('user_id')->toArray();
               $users = $users->WhereIn('id', $user_ids);
           }
        ...
        elseif(!empty($country_id)) {
            if (count($country_id) ==1){
                $user_ids = Address::where('country_id', $country_id[0])->pluck('user_id')->toArray();
                $users = $users->WhereIn('id', $user_ids);
           }
           if (count($country_id) >1){
               $user_idss = Address::where('country_id', $country_id[0]);
               for ($x = 0; $x <(count($country_id)); $x++) {
                  $user_idss = $user_idss->orWhere('country_id', $country_id[$x]);
                  }   
                $user_ids = $user_idss->pluck('user_id')->toArray();
               $users = $users->WhereIn('id', $user_ids);
           }
        ...
        if($mother_tongue_id != null){
            if (count($mother_tongue_id) ==1){
                $user_ids = Member::where('mothere_tongue', $mother_tongue_id[0])->pluck('user_id')->toArray();
               
                $users = $users->WhereIn('id', $user_ids);
            }
            if (count($mother_tongue_id) >1){
                $user_idss = Member::where('mothere_tongue', $mother_tongue_id[0]);
                for ($x = 0; $x <(count($mother_tongue_id)); $x++) {
                   $user_idss = $user_idss->orWhere('mothere_tongue', $mother_tongue_id[$x]);
                   }   
                 $user_ids = $user_idss->pluck('user_id')->toArray();
                $users = $users->WhereIn('id', $user_ids);
            }
