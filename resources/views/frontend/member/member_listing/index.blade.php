...
<script type="text/javascript">
    ...
    function get_castes_by_religion(){
      var religion_id = $("#religion_id :selected").val();
      var religion_count = $('#religion_id :selected').length;
      if (religion_count==1){
        $.post('{{ route('castes.get_caste_by_religion') }}',{_token:'{{ csrf_token() }}', religion_id:religion_id}, function(data){
          ...
          var caste_id_array = @json($caste_id);
          $("#caste_id > option").each(function() {
              if(Array.isArray(caste_id_array) ){
                  if(caste_id_array.includes(this.value)){
                  $("#caste_id").val(caste_id_array).prop("selected", true);
                }
              }
...
*/
   ...
   /* $('#caste_id').on('change', function() {
    ...
    function get_states_by_country(){
      ...
      var country_count = $('#country_id :selected').length;
      if (country_count==1){
        $.post('{{ route('states.get_state_by_country') }}',{_token:'{{ csrf_token() }}', country_id:country_id}, function(data){
          ...
          var state_id_array = @json($state_id);
          $("#state_id > option").each(function() {
              if(Array.isArray(state_id_array) ){
                  if(state_id_array.includes(this.value)){
                  $("#state_id").val(state_id_array).prop("selected", true);
                }
              }
    ...
    function get_cities_by_state(){
      ...
      if (state_count==1){
           $.post('{{ route('cities.get_cities_by_state') }}',{_token:'{{ csrf_token() }}', state_id:state_id}, function(data){
			  ...
			  var city_id_array = @json($city_id);
              $("#city_id > option").each(function() {
                  if(Array.isArray(city_id_array) ){
                  if(city_id_array.includes(this.value)){
                  $("#city_id").val(city_id_array).prop("selected", true);
                }
              }
              });
              AIZ.plugins.bootstrapSelect('refresh');
