<form action="{{ route('career.update', $career->id) }}" method="POST">
    ...
    <div class="modal-body">
        <!-- input type changed from text to select-- designation-->
        <div class="form-group row">
            <label class="col-md-3 col-form-label">{{translate('Designation')}}</label>
            <div class="col-md-9">
                <select id="designation" name="designation" class="form-control" required>
                    <option value="{{$career->designation}}" selected hidden>{{$career->designation}}</option>
        ...
        <div class="form-group row">
            ...
            <div class="col-md-9">
                <input type="text" name="company" value="{{$career->company}}"  placeholder="{{ translate('company  (optional)') }}" class="form-control" >
            </div>
        </div>
    </div>
